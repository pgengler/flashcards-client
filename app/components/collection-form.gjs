...
export default class CollectionForm extends Component {<template><form {{on "submit" (preventDefault this.save)}}>
  <div class="mb-3">
    <label for="name" class="form-label">Name:</label>
    {{!-- template-lint-disable no-builtin-form-components --}}
    <Input id="name" name="name" class="form-control {{invalidClass @collection "name"}}" @value={{this.name}} required {{autofocus}} />
