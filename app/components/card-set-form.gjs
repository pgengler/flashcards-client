...
export default class CardSetForm extends Component {<template><form {{on "submit" (preventDefault this.save)}} data-test-card-set-form ...attributes>
  <div class="container">
    <div class="row mb-4">
      <label for="name" class="form-label">Name:</label>
      <input type="text" id="name" name="name" value={{@cardSet.name}} class="form-control {{invalidClass @cardSet "name"}}" required {{autofocus}} />
