...
export default class CardComponent extends Component {<template>{{#if this.isEditing}}
  <CardForm @card={{@card}} @submitLabel="Save" @onSave={{this.exitEdit}} data-test-edit-form>
    <button type="button" class="btn btn-outline-secondary ms-3" data-test-cancel-button {{on "click" this.cancelEdit}}>
      Cancel
    </button>
...
{{else}}
  <FlippableCard @card={{@card}} @side={{this.side}} @flip={{this.flip}} class="mb-4 {{if this.isResetting "no-transition"}}" data-test-card {{didUpdate this.resetToFront @card}} ...attributes />
